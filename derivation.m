...
% Mr = sym('Mr',[3 1]); % reaction moments on the gripper in the gripper frame, at the center of actuation. Includes the effect of input torques on the two joints.
...
% OLD (INCORRECT) VELOCITY BASED KINEMATIC CONSTRAINT
% kinematic_constraint_exp = Om_d.'*Rq.'*Rg*e1 + Om.'*(-hat(Om)*Rq.'*Rg+Rq.'*Rg*hat(w))*e1 - e1.'*w_d % == 0

% NEW ROTATION BASED KINEMATIC CONSTRAINT
...
% NEW PROBABLY CORRECT VELOCITY CONSTRAINT
...
ode = @(x,u) [
  ...
  reshape(reshape(x(4:(4+8)),[3 3])*hat(x(25:27)),[9 1]);
  reshape(reshape(x(13:(13+8)),[3 3])*hat(x(28:30)),[9 1]);
  u(1)/(mg_+mq_)*reshape(x(4:(4+8)),[3,3])*e3 - g_*e3;
  compute_M_state(reshape(x(13:(13+8)),[3 3]),reshape(x(4:(4+8)),[3 3])) \ (compute_B_state(reshape(x(13:(13+8)),[3 3]),reshape(x(4:(4+8)),[3 3]))* u + compute_a_state(reshape(x(13:(13+8)),[3 3]),reshape(x(4:(4+8)),[3 3]),x(25:27),x(28:30)))
];
...
ode = @(x,u) [
  velocity_cascade*x; % first three rows
  reshape(op(x(4:(4+8)),hat(x(25:27))),[9 1]);
  reshape(op(x(13:(13+8)),hat(x(28:30))),[9 1]);
  u(1)/(mg_+mq_)*op(x(4:(4+8)),e3) - g_*e3;
  compute_M_state(op(x(13:13+8),eye(3)),op(x(4:4+8),eye(3))) \ (compute_B_state(op(x(13:13+8),eye(3)),op(x(4:4+8),eye(3)))* u + compute_a_state(op(x(13:13+8),eye(3)),op(x(4:4+8),eye(3)),x(25:27),x(28:30)))
];
